...
bool checkStratificationND(
	...
	uint32_t shift;
	uint32_t npts1D;
	unsigned char squares[1024][1024];			// [64][64] for max octave = 12	[256][256] for max octave = 16 [1024][1024] for max octave = 20

	for (uint d1 = dim1; d1 < dim2; d1++) {
		for (uint d2 = d1+1; d2 <= dim2; d2++) {
			for (uint iOctave = octaveMin; iOctave <= octaveMax; iOctave += 1) {
				if (dbg_flag) cout << ">>> checking for dims " << d1 << "," << d2 << " octave " << iOctave <<  endl;
				...
				if ((iOctave & 1) == 0) { //enven octave
					...
					for (uint ipt = 0; ipt < npts; ipt++) {
						x = sobols[d1].getSobolInt(ipt) >> shift;
						y = sobols[d2].getSobolInt(ipt)  >> shift;
						if (dbg_flag) cout << d1 << "," << d2 << " " << ipt << " : " << x << " " << y << endl;
						if (squares[y][x] == 1) {
							return false;
						}
						squares[y][x] = 1;
					...
					if(check_odd_octaves_flag) {
						...
						for (uint ipt = 0; ipt < npts; ipt++) {
							if (shift == N_SIGNIFICANT_BITS - 1) {
								...
								x = sobols[d1].getSobolInt(ipt) >> shift;
								y = sobols[d2].getSobolInt(ipt) >> (shift + 1);
							}
							if (squares[y][x] == 1) {
								return false;
							}
						...
						for (uint ipt = 0; ipt < npts; ipt++) {
							if (shift == N_SIGNIFICANT_BITS - 1) {
								...
								x = sobols[d1].getSobolInt(ipt) >> (shift + 1);
								y = sobols[d2].getSobolInt(ipt) >> shift;
							}
							if (squares[y][x] == 1) {
								return false;
...
int main(int argc, char **argv) {
	...
	int dim1=0;
    int dim2=-1;
    unsigned int nTrialsdim1 = 4*1024*1024;
    unsigned int nTrialsdim2 = 1024; // 4096 8192 16384 32768 65536
	unsigned int octaveMin = 4, octaveMax = 10;
	...
	bool dbg_flag = false;
    bool owen_permut_flag = false, max_tree_depth_32_flag = true;
    ...
    bool check_odd_octaves_flag = false;
    bool all_test_are_OK;
    int nDims = 2;
	uint32_t seed = 13374269;

	CLI::App app { "checkerStratND" };
	...
	app.add_option("--checkOddOctaves", check_odd_octaves_flag, "flag whether we need to check octaves 5,7,etc., default: " + std::to_string(check_odd_octaves_flag) );
...
#pragma omp critical (output)
        cout << "Checking Sobol indices (" << dim1 << "," << dim2 << ") between 2^" << octaveMin << " and 2^" << octaveMax << endl;

        all_test_are_OK = checkStratificationND(sobols,
		                    dim1,
		                    dim2,
                            octaveMin,
                            octaveMax,
							check_odd_octaves_flag,
		                    dbg_flag);
