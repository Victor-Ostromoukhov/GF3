...
bool checkStratificationOnePairOfDims(
	...
	bool found;
	unsigned char squares[128][128];		// for static allocation 2^20 max
...
#pragma omp critical
	std::cout << dim1 << "," << dim2 << ":   \t" << npairs << " pairs found." << std::endl;
//	outfile.close();
} // checkStratification
...
int main(int argc, char **argv) {
    ...
    omp_set_dynamic(0);     // Explicitly disable dynamic teams
    omp_set_num_threads(2); // Use 16 threads for all consecutive parallel regions#pragma omp parallel for schedule(dynamic) reduction(+:mse_accumulator)
#pragma omp parallel
	{
		t_sampler_env *sampler_env;
#pragma omp critical
		sampler_env = sampler_init(dir_vectors_fname, 0, false);
