    ...
    const Raydata& primary_hit )
{
    ...
    for(int i= 0; i <= depth; i++)
    {
        ...
        if(options.force_last_bounce == false || (options.force_last_bounce && i == depth))
...
int main( int argc, char *argv[] )
{
    ...
    // sampler sobol++ owen++ (par pixel)
    if(std::string(options.sampler_name) == "csobolpp")
        main_sampler= new CSobolPPSampler(options.dimensions, width, height, pixel_samples, options.seeds_filename, options.sobol_filename, options.owen_bits, /* seed */ seed);
...
//~ exit(1);
    ...
    // sampler ZTSequence, pbrt (0,1)
    if(std::string(options.sampler_name) == "zt")
        main_sampler= new Sampler02(options.dimensions, width, height, pixel_samples, /* seed */ seed);
...
#define THREADS
    ...
    for(int r= 0; r < options.runs; r++)
    {
        ...
        #pragma omp parallel for schedule (dynamic, 4096)
            for(int64_t i= 0; i < int64_t(main_samples); i++)
            {
            #pragma omp atomic
                samples++;
                
                if(100*samples / main_samples != 100*(samples+1) / main_samples)
                {
