...
void main(void)
{
    ...
    //~ // float y= max(color.r, max(color.g, max(color.b, 0)));	// normalisation de la couleur : (color / max(r, g, b)) == teinte
    //~ color= (color / y) * k1 * pow(y, 1.0 / compression);
    color.rgb= k1 * pow(color.rgb, vec3(1.0 / compression));
	//~ color.rgb= color.rgb * pow(2, saturation);
    //~ color.rgb= pow(color.rgb, vec3(1 / compression));
