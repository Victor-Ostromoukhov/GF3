...
void main( )
{
    ...
    cos_theta= 1;
        ...
        cos_theta= max(0, dot(normal, normalize(- vertex_position)));
    #endif
#endif
    ...
    // hachure les triangles dont les normales sont mal orientees
#if defined USE_NORMAL
    if(dot(normal, normalize(- vertex_position)) < 0)
    {
        ivec2 pixel= ivec2(gl_FragCoord.xy / 4) % ivec2(2, 2);
        if((pixel.x ^ pixel.y) == 0)
            color= vec4(0.8, 0.4, 0, 1);
