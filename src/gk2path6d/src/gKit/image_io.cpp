...
#include "image_io.h"
#include "wavefront.h"  // pathname/basemame/normalizepath, a deplacer...
    ...
    if(format.BitsPerPixel == 32)
    {
        ...
        for(int y= height -1; y >= 0; y--, py++)
        {
            ...
            for(int x= 0; x < width; x++)
            {
                ...
                image(x, y)= Color32((float) r / 255.f, (float) g / 255.f, (float) b / 255.f, (float) a / 255.f);
    ...
    else
    {
        ...
        for(int y= height -1; y >= 0; y--, py++)
        {
            ...
            for(int x= 0; x < surface->w; x++)
            {
                ...
                image(x, y)= Color32((float) r / 255.f, (float) g / 255.f, (float) b / 255.f);
                pixel= pixel + format.BytesPerPixel;
            }
        }
    }
...
int write_image( const Image& image, const char *filename )
{
    ...
    {
        Color32 color= image(x, image.height() - y -1);
