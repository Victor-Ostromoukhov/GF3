...
{
    Float sin_t= std::sin(radians(a));
    Float cos_t= std::cos(radians(a));
...
Transform Align( const Vector& v1, const Vector& v2 )
{
    Vector axis= cross(v1, v2);
    Vector a= normalize(axis);
    Float s= length(axis);      // sin_theta
    Float c= dot(v1, v2);         // cos_theta
...
Transform Perspective( const Float fov, const Float aspect, const Float znear, const Float zfar )
{
    ...
    Float itan= 1 / std::tan(radians(fov) / 2);
    Float id= 1 / (znear - zfar);

    return Transform(
        itan/aspect,    0,               0,                 0,
                  0, itan,               0,                 0,
                  0,    0, (zfar+znear)*id, 2*zfar*znear*id,
                  0,    0,              -1,                 0);
}
...
Transform Transform::inverse( ) const
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        Float pivinv = 1 / minv.m[icol][icol];
        minv.m[icol][icol] = 1;
        for (int j = 0; j < 4; j++)
