...
struct Point
{
    ...
    Point( const vec2& v );   // l'implementation se trouve en fin de fichier, la structure vec2 n'est pas encore connue.
...
inline Point::Point( const vec2& v ) : x(v.x), y(v.y), z(0) {}
...
inline vec2::vec2( const Vector& v ) : x(v.x), y(v.y) {}
