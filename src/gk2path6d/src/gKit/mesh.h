...
struct Material
{
    ...
    std::string specular_filename;      //!< nom de la texture reflechissante
    int diffuse_texture;                //!< indice de la texture
    int specular_texture;               //!< indice de la texture
    
    Material( ) : diffuse(Float(0.8), Float(0.8), Float(0.8)), specular(Black()), emission(), ns(0), 
        diffuse_filename(), specular_filename(), diffuse_texture(-1), specular_texture(-1) {}
    
    Material( const Color& color ) : diffuse(color), specular(Black()), emission(), ns(0), 
        diffuse_filename(), specular_filename(), diffuse_texture(-1), specular_texture(-1) {}
};
...
public:
    ...
    */    
    ...
    // ... ne pas utiliser ...
    void positions( const std::vector<vec3>& data ) { m_positions= data; m_update_buffers= true; }
    void texcoords( const std::vector<vec2>& data ) { m_texcoords= data; m_update_buffers= true; }
    void normals( const std::vector<vec3>& data ) { m_normals= data; m_update_buffers= true; }
    void colors( const std::vector<vec4>& data ) { m_colors= data; m_update_buffers= true; }
    void indices( const std::vector<unsigned int>& data ) { m_indices= data; m_update_buffers= true; }
    void materials( const std::vector<unsigned int>&  data ) { m_triangle_materials= data; }
    //@}
