...
GLuint Mesh::create_buffers( const bool use_texcoord, const bool use_normal, const bool use_color )
{
...
#ifndef FLOAT32
    // \todo construire un vector<float> temporaire... pas sur que les attributs double soient supportes partout
    ...
    #ifndef FLOAT32
        glVertexAttribPointer(1, 2, GL_DOUBLE, GL_FALSE, 0, (const void *) offset);
    #else
    ...
    #ifndef FLOAT32        
        glVertexAttribPointer(2, 3, GL_DOUBLE, GL_FALSE, 0, (const void *) offset);
    #else
    ...
    #else
        glVertexAttribPointer(3, 4, GL_DOUBLE, GL_FALSE, 0, (const void *) offset);
    #endif
...
int Mesh::update_buffers( const bool use_texcoord, const bool use_normal, const bool use_color )
{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, m_buffer);
    size_t length= vertex_buffer_size() + texcoord_buffer_size() + normal_buffer_size() + color_buffer_size();
    if(length > m_buffer_length)
    {
